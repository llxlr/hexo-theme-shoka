...
hexo.extend.filter.register('template_locals', locals => {
  ...
  locals.title = pangu.spacingText(__('title') !== 'title' ? __('title') : config.title);
  locals.subtitle = pangu.spacingText(__('subtitle') !== 'subtitle' ? __('subtitle') : config.subtitle);
  locals.author = __('author') !== 'author' ? __('author') : config.author;
  locals.description = pangu.spacingText(__('description') !== 'description' ? __('description') : config.description);
  ...
  if(locals.page.title) {
    locals.page.title = pangu.spacingText(locals.page.title);
  }
  locals.page.lastcat = '';
  if (locals.page.categories) {
    locals.page.categories.map((cat) => {
      if(cat.name) {
        cat.name = locals.page.lastcat = pangu.spacingText(cat.name);
      }
      return cat;
    })
  }
  ...
  if (locals.page.category) {
    locals.page.title = pangu.spacingText(locals.page.category);
  }
